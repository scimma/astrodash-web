...
def main():
    ...
    for fname in fileList:
        if fname.endswith('.lnw'):
            ...
            result = read_superfit_template(fname)
            if result[0] is None:
                continue
            wave, fluxes, nCols, ages, ttype = result
        else:
            continue
        # print(ttype, fname)
...
if __name__ == '__main__':
    main()
    # Do NOT append SLSN .dat files or any .dat files to templist.txt
    # Only .lnw files and valid Astrodash types should be included
    # Remove any .dat or SLSN entries if present (cleanup)
    with open('templist.txt', 'r') as f:
        lines = f.readlines()
    cleaned = []
    for line in lines:
        l = line.strip()
        if l and not l.endswith('.dat') and not l.startswith('SLSN/'):
            cleaned.append(l + '\n')
    with open('templist.txt', 'w') as f:
        f.writelines(cleaned)

    # Debug: print first 10 files to process
    print("First 10 files to process:", cleaned[:10])
    # Debug: process each file and print status
    for fname in cleaned:
        ...
        if fname.endswith('.lnw'):
            try:
                result = snid_template_spectra_all(fname)
                print(f"  Parsed .lnw: {fname}, result type: {type(result)}")
            except Exception as e:
                print(f"  Error parsing .lnw: {fname}, error: {e}")
        elif fname.endswith('.dat'):
            try:
                result = read_superfit_template(fname)
                print(f"  Parsed .dat: {fname}, result type: {type(result)}")
            except Exception as e:
                print(f"  Error parsing .dat: {fname}, error: {e}")
        else:
