...
def register_exception_handlers(app):
    ...
    async def file_not_found_handler(request: Request, exc: FileNotFoundError):
        ...
        return JSONResponse(
            ...
            content={"detail": f"File not found: {exc.filename}"}
        )

    @app.exception_handler(KeyError)
    async def key_error_handler(request: Request, exc: KeyError):
        logger.error(f"KeyError: {exc}", exc_info=True)
        return JSONResponse(
            status_code=HTTP_404_NOT_FOUND,
    ...
    async def import_error_handler(request: Request, exc: ImportError):
        logger.error(f"ImportError: {exc}", exc_info=True)
        return JSONResponse(
            status_code=HTTP_500_INTERNAL_SERVER_ERROR,
            content={"detail": f"Module import error: {exc}"}
        )
