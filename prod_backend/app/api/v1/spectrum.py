...
from app.core.dependencies import get_sqlalchemy_spectrum_repository, get_osc_spectrum_repo, get_model_factory, get_app_settings, get_template_analysis_service
...
async def _get_spectrum_data(
    ...
    try:
        if file:
            ...
            return await spectrum_service.get_spectrum_from_osc_ref(osc_ref)
        else:
            raise ValidationError("No spectrum file or OSC reference provided")
    except Exception as e:
        logger.error(f"Error getting spectrum data: {e}")
        raise ValidationError(f"Spectrum data error: {str(e)}")
...
async def _classify_with_user_model(
    ...
    try:
        ...
        raise ValidationError(f"User model classification error: {str(e)}")
